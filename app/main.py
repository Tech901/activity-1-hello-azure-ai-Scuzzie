        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are a Memphis 311 dispatcher. Classify the citizen complaint "
                    "into exactly one of these categories: Pothole, Noise Complaint, "
                    "Trash/Litter, Street Light, Water/Sewer, Other. "
                    "Respond with a JSON object containing exactly three keys: "
                    "category (string), confidence (float between 0.0 and 1.0), "
                    "and reasoning (short string explanation)."
                ),
            },
            {
                "role": "user",
...
def extract_key_phrases(text: str) -> list[str]:
    ...
    if response[0].is_error:
        raise RuntimeError(f"Key phrase extraction failed: {response[0].error}")
